...
MENULIST menu_ambiente[] = {
    {"Criar Nova Pasta", "", 0, 0, 4,0},
    {"Abrir Terminal", "term.bin", 0,0,0,0},
    {"", "", 2, 0, 0, 0},
    {"Sair", "", 1, 0, 0, 0},
};
...
static void *f1(){
    ...
    while(1){
        if(mouse->b&0x2){ // RIGHT BTN
            ...
            if(!sinal){
                init_window( mouse->x, mouse->y, 160, 24*MENU_SIZE+16, 0, 0xC0C0C0, 0, w1);
                sinal = 1;
                record_xy2 = recorde_xy = 8000;
                record_type = -1;
                fill_menu(menu_ambiente, MENU_SIZE,mouse->x, mouse->y, w1);
                w1->visibility = 1;
                while(mouse->b&0x2);
            }else{
                while(mouse->b&0x2);
...
static void fill_menu_data(MENULIST *menu, int count, int xy, int *r_type, WINDOW *w){
    ...
    for(int i=0; i < count; i++){
        ...
        if(xy == i){
            drawline( 4, 8+(24*i), w->width-8, 24, rgb, w);
            rgb = 0xFFFFFF;
            *r_type = menu[i].type;
...
int fill_menu(MENULIST *menu, int count, int x, int y, WINDOW *w){
    ...
    if(xy == -1 && process > 0){
        ...
        if(xy2 == -1){
            ...
            // process sumen, fill
            if(record_xy2 != xy2){
                int c = submenu[0].count;
                fill_menu_data(submenu, c, xy2, &record_type2, w2);
                record_xy2 = xy2;
                recorde_xy = -1;
            }
            return 0;
    ...
    if(recorde_xy == xy){
        if(record_type == 1 && w2->visibility == 0){ // submenu
            ...
            fill_menu_data(submenu, c, -1, &record_type2, w2);
            w2->visibility = 1;
        }
        return 0;
    }
    ...
    record_type = -1;
    recorde_xy = xy;
    fill_menu_data(menu_ambiente, count, xy, &record_type, w1);
...
static void assistant(){
    ...
    while(1){
        ...
        sendto(sd, message, strlen(message), 0, (struct sockaddr*)&serb, sizeof(struct sockaddr_in));
	}
